...
def video_detection(path_x):
    ...
    model = YOLO(r"C:\Users\PMTC-ELE\Desktop\test\app\best.pt")
    classNames = [
        "QR code scanning",
...
def home():
    global detected_objects, pass_count, fail_count
    detected_objects = []
    pass_count = 0
    fail_count = 0
    return render_template('Updated web.html', detected_objects=detected_objects, pass_count=pass_count, fail_count=fail_count)

@app.route('/update_status', methods=['POST'])
def update_status():
    global pass_count, fail_count

    is_pass = request.json.get('is_pass', False)
...
def get_section2_data():
    return render_template('Updated section3.html', class_name_dict=class_name_dict, detected_objects=detected_objects)
